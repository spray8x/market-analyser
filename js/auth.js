...
const Auth = (() => {
  ...
  // Safe to call at script-parse time — sessionStorage is synchronous and available immediately.
  // However callers should NOT rely on the return value to gate DOM work; use it only as a
  // redirect trigger. Actual rendering should always be inside DOMContentLoaded.
  function requireRole(expected) {
    const role = getRole();
    // Resolve index.html relative to current page so it works under any subdirectory
    ...
    if (expected && role !== expected) {
      window.location.replace(base + (role === 'vendor' ? 'vendor.html' : 'customer.html'));
      return false;
    }
    return true;
  }
